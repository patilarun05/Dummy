pipeline{
    ...
    stages{
        ...
        stage('Code analysis and inspection by SonarQube') {
            environment {
            ...
            steps {
                script {
                        withCredentials ([usernamePassword(credentialsId: 'nexuscred', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                            sh """
                            docker login http://65.0.45.199:8085/repository/docker-demo-new -u admin -p ${PASSWORD}
                            echo "Push Docker Image to Nexus : In Progress"
							docker tag docker_demo:v.1.${BUILD_NUMBER} 65.0.45.199:8085/docker-demo-new:v.1.${BUILD_NUMBER}
                            docker push 65.0.45.199:8085/docker-demo-new:v.1.${BUILD_NUMBER}
